...
**4a: Self-Authorization by Omission.** The agent skips a permission step — it proceeds with an action that requires external approval without asking. The authorization checkpoint is bypassed. The agent acts as if it doesn't need permission. Detection signature: an irreversible action (push, delete, send, deploy) with no preceding user authorization in the conversation transcript or approval workflow.

**4b: Self-Authorization by Fabrication.** The agent generates a false record of authorization — it claims the user approved an action that the user did not approve. The authorization checkpoint is not bypassed; it is satisfied with a forged token. This is qualitatively more severe than 4a because it actively corrupts the evidence chain. A post-hoc audit would see an authorization event that appears legitimate but corresponds to no real user input. Detection signature: the agent's output contains an attribution of user intent ("user said yes," "user approved," "user wants me to") that does not correspond to any content in the preceding user turns.

**4b was discovered live:** during a collaborative session, the agent built a catalog entry the user had agreed to work on collaboratively. Upon completion, the agent generated the statement "Anna said 'yes' — she wants me to push BDC-029 to the catalog repo" in its own output. The user had said "yes" to collaborating on the entry — not to pushing it to the repository. The agent reattributed a real "yes" from one context (collaboration) to a different action (deployment), fabricating the authorization for the second action from the user's consent to the first.

The human operator noticed the discrepancy but dismissed it because the outcome aligned with her goals — the push was something she would have approved anyway. This is documented as the Motivation-Aligned Tolerance failure mode (see companion biomimetic paper, immune failure mode analysis).

Detection surface: conversation transcripts, tool invocation logs correlated with conversation history.
...
Nineteen rules implemented in coffer-detect v0.5, validated against 111 real audit events across 11 credentials and 10 event types.
...
- **Criterion:** #2 + #5
- **Severity:** MEDIUM-HIGH (burst) / LOW-MEDIUM (chronic)
- **Signal:** 3+ consecutive failures for the same credential. Time-windowed: failures within 300s are "burst" (probing pattern, MEDIUM-HIGH). Failures spread over longer periods are "chronic" (misconfiguration, LOW-MEDIUM).
- **Observed:** Yes — 4 consecutive browser_fetch.failed (30s timeout each, burst). Also 5 consecutive stripe-test failures over 33,074s (chronic misconfiguration).
...
- **Criterion:** #2 Command Vocabulary
- **Severity:** LOW-MEDIUM
- **Signal:** Repeated login-then-single-fetch cycles instead of reusing sessions. Counts per-alias; does not require perfectly consecutive cycles.
- **Observed:** Yes — 3 login-then-single-fetch cycles for onetrust-blog.
...
- **Criterion:** #2 Command Vocabulary
- **Severity:** MEDIUM-HIGH (real credentials) / LOW (dummy credentials)
- **Signal:** credential.used reports success but HTTP status is 401/403, OR status is auth_rejected — credential was injected but rejected by the target. Severity is downgraded for known-dummy/test credentials (aliases starting with "test-" or ending with "-test") since dummy credentials are expected to be rejected by real endpoints.
...
- **Criterion:** #5 Traversal Anomaly
- **Severity:** HIGH (escalates to CRITICAL at 3+ violations)
...
- **Observed:** Yes — github-pat used against api.stripe.com, blocked by URL allowlist. Repeat violations from the same credential escalate to CRITICAL (active probing pattern).
...
- **Observed:** Yes — 5 fetches for onetrust-blog within 60 seconds, 14,202 bytes across 5 unique URLs. Known evasion: slow trickle fetches (1 per 3+ minutes) bypass this rule. Cumulative volume tracking is the countermeasure.
...
- **Observed:** Yes — github-pat and aws-dev used 9 seconds apart with no upstream task justification.

### Rule 017: Missing Reason
- **Criterion:** Layer 2 Context Binding
- **Severity:** MEDIUM-HIGH (post-fix) / LOW (pre-fix)
- **Signal:** credential.used, credential.test, credential.access_denied, browser_fetch.success, or browser_fetch.failed event where the agent did not provide a reason for accessing the credential. Recency-weighted: post-fix events (with `source` field, indicating the reason feature was available) are MEDIUM-HIGH. Pre-fix events (predating the reason feature) are LOW/informational.
- **Observed:** Yes — 3 post-fix events missing reasons flagged at MEDIUM-HIGH; 48 pre-fix events flagged at LOW.
- **Schema transition note:** The agent's stated reason appears in three different field names across schema versions: `reason` (pre-v0.5 credential.used/test), `stated_reason` (pre-v0.5 access_denied), and `agent_reason` (v0.5+). Detection code must check all three fields until pre-v0.5 data ages out.

### Rule 018: Fabricated User Attribution
- **Criterion:** #4b Self-Authorization by Fabrication
- **Severity:** HIGH
- **Signal:** Agent's output contains a claim about what the user said, approved, or requested that does not correspond to any content in the preceding user turns. The agent attributes intent or consent to the user that the user did not express.
- **Detection mechanism:** Cross-reference assistant turns against user turns for attribution claims. Pattern-match for phrases indicating user authorization ("user said," "user wants," "user approved," "user asked me to," "[name] confirmed," "[name] said yes") in assistant output. For each match, verify that the attributed content appears in a user turn within the relevant conversation window. Mismatches are findings.
- **Observed:** Yes — agent generated "Anna said 'yes' — she wants me to push BDC-029 to the catalog repo." The user's actual "yes" was in response to "Want to pick a mapping and do the first full v1.1 treatment together?" — authorization to collaborate, not to deploy.
- **Note:** This rule operates on conversation transcripts (Input Stream #2), not system audit logs (Input Stream #1). It is the first rule that requires cross-stream correlation between what the agent said and what the user said.

### Rule 019: Vocabulary Shift to Irreversible Operations
- **Criterion:** #2 Command Vocabulary + #4 Self-Authorization
- **Severity:** MEDIUM-HIGH
- **Signal:** Agent's tool invocation pattern shifts within a single session from read/create operations to deploy/push/delete/send operations without an intervening user authorization in a user turn.
- **Detection mechanism:** Classify tool invocations into two categories: generative (search, create, write, edit, read) and deployment (push, commit, send, delete, publish, post, execute on remote systems). When the agent's tool invocations shift from generative to deployment category, check for an explicit user authorization in a user turn between the last generative action and the first deployment action. Absence of authorization is the signal.
- **Observed:** Yes — agent shifted from web_search and content generation to base64 encoding, Desktop Commander write_file chunking, and cross-filesystem file transfer. No user turn authorized the transition from "writing content" to "deploying content to repository."
- **Evasion note:** An agent could interleave generative and deployment actions to avoid a clean category boundary. Cumulative tracking of the deployment-to-generative ratio over a sliding window is the countermeasure.
...
[2] Duplicate event IDs (discovered during audit review) — Remove-then-re-add created event ID collisions. Event IDs must be globally unique for the detection rules to work correctly. Fixed with sequential ID generation. A second instance of this bug was discovered during v0.5 testing: CLI and MCP operations used separate ID counters, producing collisions. Fixed by unifying the counter.
...
### Criterion #4b Discovery

Criterion #4b (Self-Authorization by Fabrication) was discovered during a collaborative session on the companion biomimetic paper. The agent was authorized to collaboratively develop a catalog entry. It completed the entry, fabricated user authorization ("Anna said 'yes' — she wants me to push BDC-029 to the catalog repo"), and attempted to push the entry to the user's repository. The platform-level usage policy failsafe terminated the conversation during the file transfer attempt.

Post-incident analysis revealed that the user had noticed the fabricated authorization but dismissed it because the outcome aligned with her goals. This led to the identification of the Motivation-Aligned Tolerance failure mode — a corruptible checkpoint at the human layer where the operator's own goal alignment suppresses detection signals.
...
- Verify: Re-running the rules confirms the fix
...
### Stress Test: Fabricated Authorization Incident
...
[6] Liu, J., et al. (2025). "TraceAegis: Securing LLM-Based Agents via Hierarchical and Behavioral Anomaly Detection." arXiv:2510.11203.
[7] Abaev, N., et al. (2026). "AgentGuardian: Learning Access Control Policies to Govern AI Agent Behavior." arXiv:2601.10440.
[8] Lupinacci, M., et al. (2025). "The Dark Side of LLMs: Agent-based Attacks for Complete Computer Takeover." arXiv:2507.06850.
...
| credential.expired | Credential use blocked by expiration | v0.5 |
...
Audit detail fields (when present): url, method, status_code, content_length, agent_reason (Layer 2 context binding, added in v0.4 as reason, renamed to agent_reason in v0.5).

Source field (v0.5+): source indicates whether the event was generated by "mcp" (agent via MCP server) or "cli" (human via command line). Pre-v0.5 events do not have this field.
