...
def rule_013_auth_status_mismatch(events: list[dict]) -> list[Finding]:
    ...
    credential.used reports success but HTTP status is 401/403 — credential
    was injected but rejected by the target server.
    NEW in v0.3."""
    findings = []
    for e in events:
        if e["event_type"] == "credential.used":
            ...
            if http_status in (401, 403) and e["status"] == "success":
                findings.append(Finding(
                    ...
                    criterion="#2 Command Vocabulary",
                    severity="MEDIUM-HIGH",
                    description=f"Credential '{e['alias']}' was used successfully (injected) but server returned {http_status}.",
...
def generate_report(findings: list[Finding], events: list[dict]) -> str:
    ...
    all_rule_ids = {f"RULE-{i:03d}" for i in range(1, 14)}
